...
/**
 * In part 2 of this lab, we will change main so that if argc is > 1 then we
 ...
 * In the second case we would print out the message "Incorrect number of
 * arguments" we should also return 1 to let the operating system know that
 * there is an error.
 *
...
int main(int argc, char **argv) {
  if (argc <= 1) {
    // no command-line arguments, run the test suite
    printf("Running the test suite...\n");
    run_test(&test_reverse, "Reverse (no arguments)\n");
    return 0;
  } else {
